...
async def prev_help(call: types.CallbackQuery, repo: Repositories, callback_data: prev_data):
...
async def next_help(call: types.CallbackQuery, repo: Repositories, callback_data: prev_data):
    page = callback_data.num
    if page >= 4:
        return await call.answer("больше ничего нет")
    ...
    await call.message.edit_text(
        Texts.gettext(f"HELP_{next_next_page}"), reply_markup=help_keyboard_inline(num=next_next_page)
    )
    return None
